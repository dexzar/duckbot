  ...
  async execute(interaction) {
    ...
    if (!guildId) {
      return interaction.reply(
        'GUILD_ID is not defined in the environment variables.'
      )
    }
    ...
    try {
      ...
      await interaction.reply({
        content: `Command \`${commandName}\` was successfully deleted.`,
        ephemeral: true
      })
    } catch (error) {
      ...
      console.error(error)
      await interaction.reply({
        content: `There was an error while deleting the command \`${commandName}\`:\n\`${error.message}\``,
        ephemeral: true
      })
    }
  }
